...
for (i in seq_along(rt)) {
  rdata[[i]] <- fortify(rt[[i]], names = "date")%>%
    mutate(session=sort(.$date)[1])
}
rdata0 <- bind_rows(rdata)%>%
  st_as_sf(coords=c("longitude", "latitude"), crs=4326)
rdata <- bind_rows(rdata0)%>%
  st_intersection(st_as_sfc(bb_boia_ssa1))
...
oc1 <- ptt%>%
  ...
  ungroup()%>%
  mutate(heart_rate_s=scale(heart_rate), cadence_cycling_s=scale(cadence_cycling))
...
m <- lmer(speed~
          ...
          rel_direction_wind*wind_speed+
            heart_rate_s*cadence_cycling_s+
            (heart_rate_s*cadence_cycling_s|session),
          data=oc1)
summary(m)
re <- (ranef(m)$session)%>%tibble::rownames_to_column("session")%>%
  mutate(session=as.POSIXct(session))
ggplot(aes(x=session,y=heart_rate_s), data=re) + geom_point() + geom_smooth(method="lm")
ggplot(aes(x=session,y=cadence_cycling_s), data=re) + geom_point() + geom_smooth(method="lm")
